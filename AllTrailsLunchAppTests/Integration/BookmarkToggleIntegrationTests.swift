...
/// AllTrailsLunchTests
///
/// Integration tests to ensure bookmark toggle works correctly across the app.
/// These tests verify the fix for the bug where multiple FavoritesManager instances
/// caused bookmark state to not sync between ViewModel and UI.
///
...
final class BookmarkToggleIntegrationTests: XCTestCase {
    ...
    func testBookmarkToggle_MultipleToggles_StateStaysSynced() {
        ...
        // Then - All should see the final state (favorited)
        XCTAssertTrue(favoritesManager.isFavorite(place.id))
    ...
    func testBookmarkToggle_ServiceFailure_StillUpdatesMemoryState() {
        ...
        // Then - Memory state should still be updated even if service fails
        XCTAssertTrue(favoritesManager.isFavorite(place.id),
    ...
    func testBookmarkToggle_LogsEventCorrectly() {
        // Given
        let place = PlaceFixtures.sampleRestaurant
        viewModel.results = [place] // Place must be in results for event to be logged
